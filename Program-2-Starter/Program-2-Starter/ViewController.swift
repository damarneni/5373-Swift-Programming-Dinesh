...
class ViewController: UIViewController , ColorTableViewControllerDelegate{
    ...
    @IBOutlet weak var h1Value: UILabel!
    @IBOutlet weak var s1Value: UILabel!
    @IBOutlet weak var lValue: UILabel!
    
    @IBOutlet weak var h2Value: UILabel!
    @IBOutlet weak var s2Value: UILabel!
    ...
    func ColorTableViewChooseColor(controller: ColorTableViewController, chosenColor color: String){
        ...
        rvalue.text = String(format:"%f", r)
        gvalue.text = String(format:"%f", g)
        bvalue.text = String(format:"%f", b)
        
        let hsl = myColors.fetchHSL(color)
        
        h1Value.text = String(format:"%d", hsl.H)
        s1Value.text = String(format:"%f", hsl.S)
        lValue.text = String(format:"%f", hsl.L)
        
        let hsv = myColors.fetchHSV(color)
        
        h2Value.text = String(format:"%d", hsv.H)
        s2Value.text = String(format:"%f", hsv.S)
        vValue.text = String(format:"%f", hsv.V)
        
        println("red: \(r)")
        last.backgroundColor = previous.backgroundColor
        previous.backgroundColor = latest.backgroundColor
